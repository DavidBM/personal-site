...
Galaxy.prototype.init = function(sCanvasDom, dCanvasDom, sCanvasCtx, dCanvasCtx, Handler, handlerContext, eventHandler) { //Initialize variables. Wait for draw calls or what-is-there calls
	...
	this.desplacedX = 0;
	this.desplacedY = 0;
	this.drawBackgroundGalaxy = true;
	...
	this.images = {
		finishedGalaxy: ResourcesImg.g().wait("finishedGalaxy", {
			handler: function () {
				self.loads.finishedGalaxy = true;
				self.requestStaticFrameFunction();
			}
		}),
		sectorsBackground: ResourcesImg.g().load("content/img/sectors.png", {
			handler: function () {
				self.loads.sectorsBackground = true;
			}
		})
	};
...
Galaxy.prototype.resetData = function() {
	...
	this.animationData.pathFindingConnectionsDistances.length = 0;
	...
	this.path.pathFindingCumulus[0] = 0;
	this.path.pathFindingCumulus[1] = 0;
	this.path.pathFindingMarkedLinks.length = 0;
...
Galaxy.prototype.setData = function(data) {
	...
	var i, maxX = 0, minX = 0, maxY = 0, minY = 0, node, system;

	//Adaptando datos: Preparando para poner el máximo y mínimo. -1 a todas las ID y referencias (MySQL empieza a contar desde 1 y no desde 0)
	if(typeof data.adapted === "undefined"){
...
Galaxy.prototype.isDataLoaded = function() {
	return this.hasData;
};

Galaxy.prototype.getSelectedNode = function() {
	if(this.dragData.selectedCumulus !== -1)
		return this.dragData.selectedCumulus;
	else
		return false;
};
...
Galaxy.prototype.hide = function(hide) {
	this.isHide = hide;
};
...
Galaxy.prototype.drawGalaxyBackground = function() {
	if(this.loads.finishedGalaxy === true && this.loads.sectorsBackground === true){
		var ctx = this.sCanvasC;
		var mult = 80;
		var secMult = 40;
		var antAlpha = ctx.globalAlpha;
		...
		ctx.globalAlpha = ctx.globalAlpha = antAlpha * 0.2;
		if(this.drawBackgroundGalaxy === true)
			this.sCanvasC.drawImage(ResourcesImg.g().get(this.images.sectorsBackground), -15800, -16000, 1500 * secMult, 1500 * secMult);
...
Galaxy.prototype.startAutoZoom = function(node, initialZoom, finalZoom, duration, desX, desY, initialAlpha, finalAlpha, funct, functMode, finishFunction) {
	if(this.animationData.cumulusOnZoom !== false) this.endAutoZoom(false);
	this.unbindEvents();
	this.requestStaticFrameFunction();
	...
	if(this.animationData.cumulusOnZoom === null) this.animationData.cumulusOnZoom = {x: this.centerX, y: this.centerY};
	else if(!Utils.isObject(this.animationData.cumulusOnZoom)) this.animationData.cumulusOnZoom = {x: this.nodes[this.animationData.cumulusOnZoom].x, y: this.nodes[this.animationData.cumulusOnZoom].y};
};
...
Galaxy.prototype.autoZoom = function() {
	...
	animationCenter = this.animationData.cumulusOnZoom;
...
Galaxy.prototype.isCumulus = function(mouseX, mouseY) {
	var cumulus, i;
	var zoom = this.zoom;
	var desplacedX = this.desplacedX;
	var desplacedY = this.desplacedY;
	var nodes = this.nodes;
	var s = this.cumulusSize;
	for (i = nodes.length - 1; i >= 0; i--) {
		cumulus = nodes[i];
		if(Math.abs(cumulus.x * zoom + desplacedX - mouseX) <= s && Math.abs(cumulus.y * zoom + desplacedY - mouseY) <= s){
			return i;
		}
	}
...
Galaxy.prototype.findPath = function(cumulus1, cumulus2) {
	...
	var way = Utils.pathFinding(cumulus1, cumulus2, [], this.nodes);
...
Galaxy.prototype.pathIndicatorAnimation = function() {
	...
	var points = Utils.getPointOfPath(actualDistance, this.path.pathFindingMarkedLinks, this.animationData.pathFindingConnectionsDistances, this.nodes, this.animationData.pathFindingActualStart); //[0]: x, [1]: y, [2]: rad
