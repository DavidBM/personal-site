 ...
 */
/**
 * Frame: viewProj + eye + sun + time (shared). Look/atm params live on **body**
 * so each planet keeps its own settings (shared frame was last-draw-wins → flash).
 */
export const PLANET_FRAME_UNIFORM_SIZE = 128;
...
export const PLANET_DISC_WGSL = /* wgsl */ `
...
fn fs_main(in : VSOut) -> @location(0) vec4<f32> {
  ...
  if (screenRpx > 0.0 && screenRpx < 48.0) {
    ...
    if (isOcean) {
      ...
      if (k == 3) {
        let band = sin(lat * 22.0 + n1 * 2.2) * 0.5 + 0.5;
        surfM = mix(albedoBase * 0.65, albedoBase * 1.2, band);
      } else {
        surfM = mix(albedoBase * 0.75, albedoBase * 1.05, n1);
      }
    }
