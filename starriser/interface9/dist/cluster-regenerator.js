...
export function generateClusterSolarSystems(cluster, numSolarSystems, nextSystemId) {
    const jumpGates = cluster.solarSystems.filter((s) => s.isJumpGate);
    if (!jumpGates.length || numSolarSystems <= 0) {
        return {
            systems: [],
            connections: [],
            nextSystemId,
    ...
    const minInterSSDist = ((2 * Math.PI * safeRadius) / Math.max(1, numSolarSystems)) *
    ...
    while (attempt < MAX_RETRIES) {
        ...
        let s = 0;
        let ssAttempt = 0;
        const maxSSA = Math.max(1, numSolarSystems) * 10;
