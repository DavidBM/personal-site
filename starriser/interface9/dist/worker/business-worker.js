...
import { TwoDotFiveDSpatialIndex } from "../services/TwoDotFiveDSpatialIndex.js";
...
export function busConstructor(bus) {
    ...
    // OP: Add a cluster: payload MUST be a POJO
    bus.on("addCluster", ({ cluster }) => {
        if (!cluster)
            return;
    ...
    // OP: Remove a cluster
    bus.on("removeCluster", ({ clusterId }) => {
    ...
    // OP: Pointer/mouse/touch event (pointer_event)
    bus.on("pointer_event", (data) => {
        if (!clusters.length)
            return;
        ...
        if (data.type === "move" || data.type === "tap") {
            ...
            if (data.type === "tap") {
                ...
                if (!withinSelectThreshold || !hit.item) {
                    ...
                    bus.send("setConnectionColors", coloring);
                    ...
                    bus.send("show_edit_handles", {
                        clusterId: hit.item.id,
                        handles: uiObjects,
                    });
                    // Remove the connection overlay when entering edit mode
                    bus.send("setConnectionColors", {});
                    // Publish edit mode change
                    if (bus._brokerPort && bus.publish) {
                        bus.publish("edit_mode_changed", {
                            clusterId: hit.item.id,
    ...
    // Forward selection state changes back to UI for rendering overlays/decorations
    SelectionService.subscribe(({ hoveredId, selectedId }) => {
        bus.send("update_ui_state", { hoveredId, selectedId });
    ...
    // OP: Get a cluster by id (for UI, BFS, etc)
    bus.on("get_cluster_by_id", ({ id }) => {
        const out = clusters.find((c) => c.id === id) ?? null;
        bus.send("cluster_by_id", { id, cluster: out });
    });
    // OP: Allows on-demand reindexing (e.g., after batch ops)
    bus.on("rebuild_index", () => {
        clusterIndex.build(clusters);
    });
    ...
    function clearUIObjects() {
        if (uiObjects.length > 0 && currentlyEditingClusterId != null) {
            bus.send("hide_edit_handles", { clusterId: currentlyEditingClusterId });
        }
        uiObjects = [];
        uiObjectIndex.clear();
    }
    ...
    bus.on("ops", (ops) => {
        ...
        for (const op of ops) {
            if (op.type === "addCluster") {
                // Already handled by existing addCluster listener
    ...
    bus.on("setup_broker_port", () => {
        ...
        // Wait a bit for the broker port to be fully set up
        setTimeout(() => {
