...
export function clientToBufferPx(canvas, clientX, clientY) {
    ...
    if (!rect)
        throw new Error("Worker focus requires toBufferPx");
...
export function createSystemFocusController(opts) {
    ...
    const lockBody = (index) => {
        ...
        // so it can resume the current selection when follow ends.
        ...
        if (cam && typeof cam.setSystemOrbitFocus === "function") {
            cam.setSystemOrbitFocus({
                ...
                z: world.z,
                radius: boom,
                minRadius: systemOrbitMinRadius(store.radius[index], st.near),
            });
        }
    };
    const clearFocus = () => {
    ...
    return {
        tick() {
            const store = view.solarBodies;
            if (store.systemId == null || store.currentCount <= 0 || locked == null) {
        ...
        tryPickBody(clientX, clientY) {
            ...
            if (store.currentCount <= 0)
                return false;
            const buf = opts.toBufferPx
                ? opts.toBufferPx(clientX, clientY)
                : clientToBufferPx(view.canvas, clientX, clientY);
            const st = view.getCameraState();
...
//# sourceMappingURL=system-focus.js.map