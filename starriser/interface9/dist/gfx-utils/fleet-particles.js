...
export class FleetParticles {
    ...
    update(now, cameraHeight) {
        ...
        this.lodMesh.visible = false;
        if (this.activeListDirty) {
            this.rebuildActiveFleetList();
        }
        if (now >= this.nextChunkTime) {
            this.scheduleNextChunk(now);
        }
        this.updateFleetPositionsPartial(now);
    ...
    updateFleetPositionsPartial(now) {
    ...
    updateCooldownRings(now) {
        ...
        let cursor = 0;
        for (const slot of this.cooldownSlots) {
    ...
    rebuildActiveFleetList() {
        this.activeFleetIds = [];
        for (const [id, fleet] of this.fleets.entries()) {
            if (fleet.state.state === "jumping" || fleet.state.state === "cooldown") {
                this.activeFleetIds.push(id);
            }
        }
        this.activeListDirty = false;
    }
    scheduleNextChunk(now) {
        this.scheduledFleetIds = this.activeFleetIds.slice();
        this.scheduleCursor = 0;
...
//# sourceMappingURL=fleet-particles.js.map