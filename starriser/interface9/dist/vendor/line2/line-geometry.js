 ...
 */
export function buildTemplateInterleaved() {
 ...
 */
export function packSegmentPositions(array) {
 ...
 */
function expandPolylineTriples(src, label, unit) {
    if (src.length % 3 !== 0) {
        throw new Error(`${label}: length ${src.length} is not a multiple of 3 (${unit} per vertex)`);
    }
    if (src.length < 3) {
        throw new Error(`${label}: need ≥1 ${unit} triple (length ≥ 3); got ${src.length} floats`);
    }
 ...
 */
export function polylineToSegments(polyline) {
    const src = polyline instanceof Float32Array
        ? polyline
        : Float32Array.from(polyline);
 ...
 */
export function polylineColorsToSegments(colors) {
    const src = colors instanceof Float32Array
        ? colors
 ...
 */
export function validateSegmentColorCount(colors, segmentCount) {
    const len = colors.length;
    const expected = segmentCount * LINE2_COLOR_FLOATS;
    if (len !== expected) {
        throw new Error(`validateSegmentColorCount: expected ${expected} floats ` +
 ...
 */
export function packSegmentColors(array, segmentCount) {
    const out = array instanceof Float32Array
        ? array
 ...
 */
export function computeLineDistances(positions) {
...
//# sourceMappingURL=line-geometry.js.map