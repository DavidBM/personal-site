...
export function linePositionSegments(
  positions: ArrayLike<number>, polyline: boolean, splitPosition: boolean,
): ArrayLike<number> {
  if (!splitPosition) return polyline ? polylineToSegments(positions) : packSegmentPositions(positions);
  if (!polyline) {
    if (positions.length % 6 !== 0) throw new Error('Line2 positions must contain xyz segment pairs');
    return positions;
  }
  if (positions.length < 3 || positions.length % 3 !== 0) throw new Error('Line2 polyline must contain xyz triples');
  const pairs = new Float64Array(Math.max(0, positions.length - 3) * 2);
  for (let i = 0; i < positions.length - 3; i += 3) {
    for (let axis = 0; axis < 6; axis++) pairs[i * 2 + axis] = positions[i + axis]!;
  }
  return pairs;
}

/** One existing position buffer: high start/end followed by low start/end. */
export function packSplitLinePositions(positions: ArrayLike<number>, scratch: Float32Array): Float32Array {
