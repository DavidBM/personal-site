...
Canvas.prototype.phase2 = function() {
	...
	for (var i = this.data.cumulusPositionCache.length - 1; i >= 0; i--) {
		this.data.cumulusPositionCache[i] = {x: this.data.cumulus[i].x, y: this.data.cumulus[i].y, w: this.data.cumulus[i].w, h: this.data.cumulus[i].h};
	}
...
Canvas.prototype.phase3CheckIntersection = function(i, temp, distances) {
	...
	for (var j = this.data.graph.length - 1; j >= 0; j--) {
		if(this.lineIntersect(
			this.data.cumulus[i].x, this.data.cumulus[i].y, this.data.cumulus[ distances[temp].index ].x, this.data.cumulus[ distances[temp].index ].y, 
			this.data.cumulus[ this.data.graph[j].firstNode ].x, this.data.cumulus[ this.data.graph[j].firstNode ].y, this.data.cumulus[ this.data.graph[j].secondNode ].x, this.data.cumulus[ this.data.graph[j].secondNode ].y)
			){
			return {
...
Canvas.prototype.loadImages = function() {
	...
	var imageUrls = ["cumulo_alta_resolucion.png"];
...
Canvas.prototype.events = function() {
	...
	this.eventsData.zoom_factor = 0.01;
...
Canvas.prototype.isCumulus = function(mouseX, mouseY) {
	if(this.phase >= 2){
		...
		if(this.eventsData.zoom_factor < 0.039){
			...
			for (var i = this.data.cumulus.length - 1; i >= 0; i--) {
				cumulus = this.data.cumulus[i];
				if(Math.abs(cumulus.x * this.eventsData.zoom_factor + this.eventsData.desplacedX - mouseX) <= cumulus.w / 2 * this.eventsData.zoom_factor && Math.abs(cumulus.y * this.eventsData.zoom_factor + this.eventsData.desplacedY - mouseY) <= cumulus.h / 2 * this.eventsData.zoom_factor ){
					//console.log("Cumulus selected: " + i);
					return i;
				}
			};
...
Canvas.prototype.zoomSystem = function(e, delta, deltaX, deltaY) {
	...
	if(delta > 0){
		...
		if(this.eventsData.zoom_factor * growth > 5000){
			growth =  5000 / this.eventsData.zoom_factor;
			this.eventsData.zoom_factor = 5000;
		}else{
		...
		if(this.eventsData.zoom_factor / growth < 0.01){
			growth = this.eventsData.zoom_factor / 0.01;
			this.eventsData.zoom_factor = 0.01;
		}else{
			this.eventsData.zoom_factor /= growth;
		}
...
Canvas.prototype.manualZoomInOut = function(zoom) {
	...
	if(zoom == "+"){
		if(this.eventsData.zoom_factor < 150){
			...
			if(this.eventsData.zoom_factor * growth > 8000){
				growth =  8000 / this.eventsData.zoom_factor;
				this.eventsData.zoom_factor = 8000;
			}else{
				this.eventsData.zoom_factor *= growth;
			}
...
Canvas.prototype.drawSystem = function() {
		...
		if(this.phase > 1 && this.eventsData.contentLoaded) this.drawSystemCumulus();
...
Canvas.prototype.updateCumulusPositions = function() {
	...
	if(this.data.cumulusPositionCache.length != temp){
		this.data.cumulusPositionCache = new Array(this.data.cumulus.length);
		for (var i = this.data.cumulusPositionCache.length - 1; i >= 0; i--) {
			this.data.cumulusPositionCache[i] = {x: 0, y: 0, w: 0, h: 0};
		}
	}
	...
	for (var i = temp - 1; i >= 0; i--) {
		...
		this.data.cumulusPositionCache[i].w = Math.floor(this.data.cumulus[i].w * this.eventsData.zoom_factor);
		this.data.cumulusPositionCache[i].h = Math.floor(this.data.cumulus[i].h * this.eventsData.zoom_factor);
	}
};

Canvas.prototype.drawSystemCumulus = function() {

	var w, h, w2, h2, cumulusImage;

	w2 = Math.round(Math.floor(this.data.cumulusPositionCache[0].w / 2));
	h2 = Math.round(Math.floor(this.data.cumulusPositionCache[0].h / 2));
	...
	if(this.eventsData.zoom_factor < 0.039){
		...
		w = this.data.cumulusPositionCache[0].w;
		h = this.data.cumulusPositionCache[0].w;
		cumulusImage = this.img.predefinedCumulusBig;
	}
	...
	for (var i = this.data.cumulusPositionCache.length - 1; i >= 0; i--) {
		if(
			this.data.cumulusPositionCache[i].x + w2 > 0 &&
			this.data.cumulusPositionCache[i].x - w2 < this.data.viewportSize.x &&
			this.data.cumulusPositionCache[i].y + h2 > 0 &&
			this.data.cumulusPositionCache[i].y - h2 < this.data.viewportSize.y){

			
			//this.contexto.drawImage(this.img[i].image, this.data.cumulusPositionCache[i].x - w2, this.data.cumulusPositionCache[i].y - h2, this.data.cumulusPositionCache[i].w, this.data.cumulusPositionCache[i].h);
			if(this.eventsData.selectedCumulus === i){
				...
				this.contexto.beginPath();
				this.contexto.arc(this.data.cumulusPositionCache[i].x, this.data.cumulusPositionCache[i].y, this.data.cumulusPositionCache[i].w / 1.8, 0, Math.PI * 2, false);
				this.contexto.fill();
			...
			this.contexto.beginPath();
			//this.contexto.arc(this.data.cumulusPositionCache[i].x, this.data.cumulusPositionCache[i].y, this.data.cumulusPositionCache[i].w / 2, 0, Math.PI * 2, false);
			//this.contexto.rect(this.data.cumulusPositionCache[i].x, this.data.cumulusPositionCache[i].y, this.data.cumulusPositionCache[i].w / 2, this.data.cumulusPositionCache[i].h / 2);
			this.contexto.drawImage(
				cumulusImage, 
				this.data.cumulusPositionCache[i].x - w2, 
				this.data.cumulusPositionCache[i].y - h2, 
				w, 
				h);
			this.contexto.stroke();
...
Canvas.prototype.drawSystemCumulusLinks = function() {
	...
	var w2, h2, multiplier, cumulus1, cumulus2, temp;
