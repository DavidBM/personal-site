...
AssistPanel.prototype.init = function(handler, context) {
	...
	var temp = ["assistPanelButtonPlanets", "assistPanelButtonHome", "assistPanelButtonZoomSystem active", "assistPanelButtonZoomCumulus", "assistPanelButtonZoomGalaxy"]
...
AssistPanel.prototype.bindEvents = function() {
	var self = this;
	var buttonsActions = ["", Utils.enums.zoomLevels.planet, Utils.enums.zoomLevels.system, Utils.enums.zoomLevels.cumulus, Utils.enums.zoomLevels.galaxy];
	var i;
	...
	for (i = 1; i < 5; i++) {
		$(this.dom.botones[i]).click((function (i) {
			return function () {
				if(self.handler.call(self.context, Utils.enums.interfaceActions.zoom, buttonsActions[i])){
