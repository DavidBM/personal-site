...
//Tiene el dragOnIdentifier que hace que solo pueda haber un drag iniciado a la vez
...
EventHandler.prototype.drag = function(event, data){
	...
	switch(event.type){
		...
		break;
		case "mousemove":
			if(this.dragOn && this.dragOnRandIdentifier == data.randIdentifier && time - data.time > 10){ //Estamos en arrastre y además es el evento que disparó el dragOn
				data.time = time;
		...
		break;
		case "touchmove":
			if(this.dragOn && this.dragOnRandIdentifier == data.randIdentifier && time - data.time > 10){
				data.time = time;
