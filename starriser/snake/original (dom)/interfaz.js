...
Interfaz.prototype.loop = function() {
	...
	if(this.data.levels[this.data.map_selected].end_position[0] < 0){
		...
		this.dom.canvas.append($("<div>").addClass("cuadro_winnerpoint").css({width: this.data.ancho_cuadro, height:this.data.alto_cuadro, left: this.data.ancho_cuadro * this.data.levels[this.data.map_selected].end_position[0], top: this.data.alto_cuadro * this.data.levels[this.data.map_selected].end_position[1]}));
	}
	this.loop_timer = setInterval(function () { //En esta función actualizamos las cosas.
		if(!loopExec){
			...
			//Y con esto hemos hecho una ronda. Ahora tendriamos que mirar cuanto se ha tardado en hacer la ronda. Esos valores se usarán más adelante. Ahora un setTimeout simple
			loopExec = false;
		}
	}, 25)
};
...
Interfaz.prototype.staticDom = function() {
	...
	this.dom = {
		canvas: $("<div>").addClass("ventana_juego").css({display: "none"}),
