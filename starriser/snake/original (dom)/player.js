...
function Player (initial_position, block_height, block_width, initial_length) {
	...
	this.data = { //Ponemos la cantidad de 
		...
		levels_radiosity_class: ["", "snake_block_l1", "snake_block_l2", "snake_block_l3"],
		level_radiosity: 0,
		velocity: 7,
	...
	//Crear la serpiente
	this.dom.snake = new Array(MAX_LENGHT);
	for (var i = 0; i < this.data.blocks; i++) {
		this.data.positions[i] = [initial_position[0], initial_position[1]];
		this.dom.snake[i] = this.dom.block.clone().css({left: this.data.positions[i][0] * block_height, top: this.data.positions[i][1] * block_width});
		this.dom.container.append(this.dom.snake[i]);
	}
	this.set_radiosity();
	this.set_velocity();
}

Player.prototype.get_dom = function() {
