"use strict";
function Walker_enemy (objs, myOrders, canvas, interfaz) {
	this.constructor(objs, myOrders, canvas, interfaz);	
}

Walker_enemy.prototype.constructor = function(objs, myOrders, canvas, interfaz) { //Objs (wall, isotopes, enemies, player)
	...
	/*this.dom = {
		enemy: $("<div>").addClass("walker_block").css({left: this.data.position[0] * this.data.widths[0], top: this.data.position[1] * this.data.widths[1], width: this.data.widths[0], height: this.data.widths[1] })
	};*/
	this.counter = {
		backsteps: 0,
		step: 1,
...
Walker_enemy.prototype.step = function() {
...
Walker_enemy.prototype.nextPosition = function() {
	var coord_temp;
	if(!this.counter.next_position_caculated){
		if(this.counter.backsteps <= 0){
			var rad, temp, coordWorm, direc, coord_temp;
			var coord_temp2 = [this.data.position[0],this.data.position[1]];
			coordWorm = this.data.objs.player.next_position();
			rad = Math.atan2( coordWorm[1] - this.data.position[1], coordWorm[0] - this.data.position[0]);
			...
			if(this.data.objs.wall.isWall(coord_temp) || this.data.objs.enemies.isEnemy(coord_temp)){ //Nos hemos encontrado contra un muro, así que corregimos. Iremos aleatoriamente a la derecha o a la izquierda. Y si no se puede. Hacia atrás. Y si no se puede. Entonces borramos este enemigo porque está bloqueado.
				...
				if(this.data.objs.wall.isWall(coord_temp2) || this.data.objs.enemies.isEnemy(coord_temp2)){ //Si la nueva posición también está bloqueada... cogemos la otra.
					...
					if(this.data.objs.wall.isWall(coord_temp2) || this.data.objs.enemies.isEnemy(coord_temp2)){//Quiere decir que adelante, derecha e izquierda estan bloqueados. Comprobamos a ver si hacia atras funciona y activamos el contador para hacerlo 5 veces.
...
Walker_enemy.prototype.draw = function(coordinates) {
	this.data.position = coordinates;
	//this.dom.enemy.css({left: this.data.position[0] * this.data.widths[0], top: this.data.position[1] * this.data.widths[1], width: this.data.widths[0], height: this.data.widths[1]});
	Ctx.fillStyle = "#112970";
...
Walker_enemy.prototype.destructor = function() {
