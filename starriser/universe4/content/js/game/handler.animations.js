Handler.prototype.mapModeInitChange = function(to, duration, funct, _toNode, toNodeZoom, last) {
	...
	var fromNode, toNode, from, selectedNode;
	...
	if(from < to){ //Zoom+ //Como he definido los enum yo, se que un nivel superior de zoom es un valor menor.
		selectedNode = fromNode = (this.selectZoneData(from).nodeSelected !== -1) ? this.selectZoneData(from).nodeSelected : 1; //Hasta que se implemente la parte de manterner un planeta seleccionado en todo los niveles de zoom
		toNode = null;
		if(to == Utils.enums.zoneWorkMode.PLANET) toNode = (this.selectZoneData(to).nodeSelected !== -1) ? this.selectZoneData(to).nodeSelected : 1;
	}else{
		fromNode = null;
		selectedNode = toNode = (this.selectZoneData(to).nodeSelected !== -1) ? this.selectZoneData(to).nodeSelected : 1; //Hasta que se implemente la parte de manterner un planeta seleccionado en todo los niveles de zoom
	}
	...
	if(from === Utils.enums.zoneWorkMode.GALAXY){
		...
		if(to === Utils.enums.zoneWorkMode.CUMULUS){
			...
			this.obj.system.startAutoZoom(null, null, toNodeZoom, duration, this.data.canvasWidth/2, this.data.canvasHeight/2, 0, 1, Utils.transformRangEaseIn, "sixt", functToNtimes);
			...
			this.obj.system.moveTo(x, y, 0.000001);
			this.obj.system.startAutoZoom(toNode, null, toNodeZoom, duration, this.data.canvasWidth/2 - InterfaceCenterDisplacement, 300, 0, 1, Utils.transformRangEaseIn, "sixt", functToNtimes, true, true);
		...
		if(to === Utils.enums.zoneWorkMode.GALAXY){
			...
			this.obj.galaxy.moveTo(selectedNode, 10, x, y);
			this.obj.galaxy.startAutoZoom(toNode, null, toNodeZoom, duration, this.data.canvasWidth/2, this.data.canvasHeight/2, 0, 1, Utils.transformRangEaseOut, "sixt", functToNtimes);
			...
			this.obj.system.moveTo(x, y, 0.00000001);
			this.obj.system.startAutoZoom(toNode, null, 0.05, duration*2, this.data.canvasWidth/2, this.data.canvasHeight/2, 0, 1, Utils.transformRangEaseInOut, "third", functToNtimes);
		...
		if(to === Utils.enums.zoneWorkMode.GALAXY){
			...
			this.obj.cumulus.startAutoZoom(toNode, null, toNodeZoom, duration, this.data.canvasWidth/2, this.data.canvasHeight/2, 0, 1, Utils.transformRangEaseOut, "sixt", functToNtimes);
		...
		if(to === Utils.enums.zoneWorkMode.GALAXY){
			...
			this.obj.cumulus.startAutoZoom(toNode, null, toNodeZoom, duration, this.data.canvasWidth/2, this.data.canvasHeight/2, -1, 1, Utils.transformRangEaseOut, "third", functToNtimes);
