...
Cumulus.prototype.init = function(sCanvasDom, dCanvasDom, sCanvasCtx, dCanvasCtx, Handler, handlerContext, eventHandler) { //Initialize variables. Wait for draw calls or what-is-there calls
	...
	this.loads = {
		spriteStructuresCT: false
	};

	this.images = {
		spriteStructuresCT: ResourcesImg.g().load("content/img/spriteStructuresCT.png", {
			handler: function () {
				self.loads.spriteStructuresCT = true;
			}
		})
	};
...
Cumulus.prototype.setData = function(data) {
	...
	this.data = data;
	var i, maxX = 0, minX = 0, maxY = 0, minY = 0, j, system;
	...
	if(typeof data.adapted === "undefined"){
		...
		for (j = this.graph.length - 1; j >= 0; j--) {
			system = this.graph[j];
			system.id--;
			system.n1--;
			system.n2--;
			system.in = j;
			this.nodes[system.n1].conn.push(system);
			this.nodes[system.n2].conn.push(system);
		}
	}
...
Cumulus.prototype.resetData = function() {
...
Cumulus.prototype.getSelectedNode = function() {
...
Cumulus.prototype.requestStaticFrameFunction = function() {
	if(this.isHide !== true) this.handler.call(this.handlerContext, Utils.enums.cumulusActions.requestStaticFrameFunction);
};
...
Cumulus.prototype.drawSolarSystems = function() {
	...
	var s, s2, cumulusImage, starGateImage;

	s = this.cumulusSize / this.zoom;
	s2 = Math.round(s / 2 );
	cumulusImage = this.cumulusIcon.canvas;
	if(this.loads.spriteStructuresCT) starGateImage = ResourcesImg.g().get(this.images.spriteStructuresCT);
	else starGateImage = false;
	...
	for (var i = this.nodes.length - 1; i >= 0; i--) {
		/*if(
			...
			}else if(starGateImage !== false){
				this.sCanvasC.drawImage(
					starGateImage,
					this.nodes[i].x - s,
					this.nodes[i].y - (s * 2 * (229 / 450)) / 2,
					s * 2,
					s * 2 * (229 / 450)
				);
			}
...
Cumulus.prototype.getPositionAndDataById = function(id) {
	if(id === null)
		return {x: this.centerX, y: this.centerY, zoom: this.zoom, desplacedX: this.desplacedX, desplacedY: this.desplacedY};
	else
		for (var i = this.nodes.length - 1; i >= 0; i--) {
			if(this.nodes[i].id === id) return {x: this.nodes[i].x, y: this.nodes[i].y, zoom: this.zoom, desplacedX: this.desplacedX, desplacedY: this.desplacedY, index: i};
		}
};


Cumulus.prototype.getIndexById = function(id) {
	for (var i = this.nodes.length - 1; i >= 0; i--) {
